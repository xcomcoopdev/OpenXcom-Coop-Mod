"""PRD-J07: SHARED facility build / dismantle + rename + sack via shared_cmd.

A SHARED campaign is one host-authoritative world. The Basescape facility screens
mutate NOTHING locally: placing a facility, dismantling one, renaming a base and
sacking a soldier each ride the J03 shared_cmd protocol. The host validates
against the live world (placement rules exactly as PlaceFacilityState), applies,
and broadcasts shared_apply; replicas apply ONLY from shared_apply.
  ...
  CONFLICT  both players build on the SAME tile in one pump -> host order
            decides: ONE succeeds, ONE gets shared_fail, worlds identical, funds
...
Run:  python tools/coop_test/test_shared_facilities.py
"""
...
def main():
    js = shared_fixture.bring_up("jfac", (48750, 48751, 48050))
    ...
    try:
        ...
        #    decides; loser gets shared_fail; worlds identical; one debit.
        ...
        host.ok({"cmd": "shared_cmd", "jcmd": "fac_build", "baseId": 0,
        ...
        assert sum(fails) >= 1, "no shared_fail surfaced for the losing build"
        ...
        client.ok({"cmd": "base_rename", "base": old_name, "name": "Shared HQ"})
        host.wait_for("host sees rename",
                      lambda: (_base0(host)["name"] == "Shared HQ") or None,
                      timeout=30, interval=0.5)
        client.wait_for("client sees rename",
                        lambda: (_base0(client)["name"] == "Shared HQ") or None,
                        timeout=30, interval=0.5)
        print(f"PASS rename: '{old_name}' -> 'Shared HQ' on both")

        # ================================================================
        # 5) SACK: client sacks a soldier by id -> removed on both sides.
        # ================================================================
        rep_h = host.ok({"cmd": "base_report", "base": "Shared HQ"})
        rep_c = client.ok({"cmd": "base_report", "base": "Shared HQ"})
        ...
        victim = ids_h[0]
        n0 = len(ids_h)
        client.ok({"cmd": "sack", "soldierId": victim, "base": "Shared HQ"})

        def _sacked(gc):
            rep = gc.ok({"cmd": "base_report", "base": "Shared HQ"})
        ...
        print("ALL SHARED FACILITIES TESTS PASSED")
    finally:
        js.shutdown()
