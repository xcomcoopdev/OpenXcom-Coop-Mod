...
(.agents/prds/shared/session-notes-gap2.md) concluded that "who flies the HK
dogfight" was undefined because craft control in SHARED is last-command-wins with
...
Sequence:
  * spawn an armed Avenger on BOTH machines (ids lock-step, the test_shared_craft
    precedent);
...
Assertions (PRD-DF01):
  ...
  3. the fight resolves to ONE authoritative outcome: after the host's dogfight
     ...
     world-equality is exercised by test_shared_intercept_spectate.py (non-HK).

Run:  python tools/coop_test/test_shared_hk_dogfight.py
"""
...
def main():
    js = shared_fixture.bring_up("jhk", (48764, 48765, 48064))
    ...
    try:
        ...
        js.finish()
        print("ALL SHARED HK SPECTATE TESTS PASSED")
    finally:
        js.shutdown()
