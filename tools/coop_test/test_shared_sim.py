...
In a SHARED campaign only the HOST runs world simulation (daily/monthly ticks,
...
replica's clock still advances (host `time` packet) and its globe still draws.
  ...
  AC3  positions: a craft launched on the host MOVES on the replica via the
       SHARED position snapshot (the replica never simulates it).

Run:  python tools/coop_test/test_shared_sim.py
"""
...
def main():
    js = shared_fixture.bring_up("jsim", (48690, 48691, 47990))
    ...
    try:
        ...
        print("ALL SHARED SIM TESTS PASSED")
    finally:
        js.shutdown()
