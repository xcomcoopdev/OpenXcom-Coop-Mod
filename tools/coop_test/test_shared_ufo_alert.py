...
A SHARED campaign runs ONE host-authoritative geoscape sim, so UFO detection only
...
the SEPARATE "mirror of the peer's UFO" flag. A SHARED client's UFO is a
...
The fix accepts the shared replica in SHARED.
...
Run:  python tools/coop_test/test_shared_ufo_alert.py
"""
...
def main():
    js = shared_fixture.bring_up("jufo", (48800, 48801, 48100))
    ...
    try:
        ...
        print("ALL SHARED UFO-ALERT TESTS PASSED")
    finally:
        js.shutdown()
