...
Run:  python tools/coop_test/test_joint_lobby_details.py
"""
...
def main():
    # ---- Build a JOINT save so we have a registered roster to resume. -----------
    js = joint_fixture.bring_up("jlob", (48936, 48937, 48236))
    ...
    try:
        newhost.spawn()
        newhost.connect()
        newhost.ok({"cmd": "open_new_game", "mode": "coop_joint"})
    ...
    try:
        ...
        print("ALL JOINT LOBBY-DETAILS TESTS PASSED")
    finally:
        rehost.shutdown()
