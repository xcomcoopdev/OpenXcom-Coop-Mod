"""GAP-6b: a SELL-flagged SHARED production must not drift the replica's stores.

A SHARED campaign is one host-authoritative world. When a production has the
...
shared_apply, so they stay synced; but the prod_done broadcast used to carry no
...
covered by test_shared_manufacture / test_shared_sim; this test targets the
item-sell drift specifically.

Run:  python tools/coop_test/test_shared_production_sell.py
"""
...
def _chk_items(gc):
    return gc.ok({"cmd": "shared_checksum"})["chkItems"]


def _resync_requests(gc):
    return gc.ok({"cmd": "shared_resync_stats"})["requests"]
...
def main():
    js = shared_fixture.bring_up("jsell", (48750, 48751, 48050))
    ...
    try:
        ...
        print("ALL SHARED PRODUCTION-SELL TESTS PASSED")
    finally:
        js.shutdown()
