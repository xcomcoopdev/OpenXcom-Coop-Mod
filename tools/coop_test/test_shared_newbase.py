"""PRD-J07: SHARED subsequent-base creation end-to-end via base_new.

Any player may Build New Base in a SHARED campaign. The CLIENT drives the full
...
submits ONE atomic base_new shared_cmd. The host validates funds + region,
...
the same index on every machine (base add rides shared_apply = index lock-step).
...
Run:  python tools/coop_test/test_shared_newbase.py
"""
...
def main():
    js = shared_fixture.bring_up("jnb", (48760, 48761, 48060))
    ...
    try:
        ...
        # same coopbaseid (host-minted, serialized into the shared_apply payload)
        ...
        print("ALL SHARED NEWBASE TESTS PASSED")
    finally:
        js.shutdown()
