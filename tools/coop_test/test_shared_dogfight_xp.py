...
THE FIX (DF01): the HOST simulates every SHARED dogfight; awardExperienceToPilots()
...
Run:  python tools/coop_test/test_shared_dogfight_xp.py
"""
...
def main():
    js = shared_fixture.bring_up("jdfxp", (48794, 48795, 48094))
    host, client = js.host, js.client
    try:
        b0 = _base0(host)
        blon, blat = b0["lon"], b0["lat"]

        # Armed craft on BOTH machines (ids lock-step, the test_shared_craft precedent).
        ...
        js.finish()
        print("ALL SHARED DOGFIGHT XP (GAP-7) TESTS PASSED")
    finally:
        js.shutdown()
