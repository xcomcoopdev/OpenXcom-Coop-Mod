"""PRD-J10: open SHARED screens react to the other player's shared_apply.
...
list). In a SHARED campaign a peer's shared_apply lands underneath an open screen
and leaves it showing a world that no longer exists. J10 binds each SHARED screen
...
broken.
  ...
  STALE     a purchase order built against the pre-apply world, submitted on the
            raw shared_cmd lane (i.e. bypassing the rebuild that would have
...
Run:  python tools/coop_test/test_shared_refresh.py
"""
...
def main():
    js = shared_fixture.bring_up("jref", (48740, 48741, 48040))
    ...
    try:
        ...
        #    Raw shared_cmd lane = the J07 tile-conflict idiom: it is the host
        ...
        client.ok({"cmd": "shared_cmd", "jcmd": "buy", "baseId": 0,
        ...
        print("ALL SHARED REFRESH TESTS PASSED")
    finally:
        js.shutdown()
