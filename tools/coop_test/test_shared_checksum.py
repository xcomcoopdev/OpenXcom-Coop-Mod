...
prod_done overshoot (GAP-6) the design still carries. test_shared_world_equal's
...
was already caught by the funds field; that path is test_shared_resync.)

Run:  python tools/coop_test/test_shared_checksum.py
"""
...
def main():
    js = shared_fixture.bring_up("jchk", (48810, 48811, 48110))
    ...
    try:
        ...
        print(f"PASS auto-detect: {_resync(client)['mismatches']} mismatch(es) -> "
              "shared_resync_request sent (checksum, not force_resync)")
        ...
        print("ALL SHARED CHECKSUM TESTS PASSED")
    finally:
        js.shutdown()
