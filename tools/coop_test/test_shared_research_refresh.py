...
The fix binds NewResearchListState to the shared_apply stream: a res_start apply
...
now-unavailable topic disappears.
  ...
  START  host starts topic T (res_start shared_cmd -> apply on both).
...
Run:  python tools/coop_test/test_shared_research_refresh.py
"""
...
def main():
    js = shared_fixture.bring_up("jresref", (48770, 48771, 48070))
    ...
    try:
        ...
        # ---- START: host starts T (res_start -> shared_apply on both) ---------
        ...
        print("ALL SHARED RESEARCH-REFRESH TESTS PASSED")
    finally:
        js.shutdown()
