...
A SHARED campaign is one host-authoritative world. Funds ride every shared_apply
...
GREEN (post-fix): the shared_apply carries the host's authoritative income/
...
Run:  python tools/coop_test/test_shared_graphs.py
"""
...
def main():
    js = shared_fixture.bring_up("jgraph", (48770, 48771, 48070))
    ...
    try:
        ...
        # (an in-flight shared_apply is a legitimate transient skew - same reason the
        ...
        print("ALL SHARED GRAPH TESTS PASSED")
    finally:
        js.shutdown()
