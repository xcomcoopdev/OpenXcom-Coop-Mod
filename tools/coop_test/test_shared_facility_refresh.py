...
think(), so before the fix a peer's fac_build shared_apply left the funds/grid
...
correct underneath.
  ...
  APPLY  host builds a facility (funds debited on the shared world, shared_apply).
...
Run:  python tools/coop_test/test_shared_facility_refresh.py
"""
...
def main():
    js = shared_fixture.bring_up("jfacref", (48760, 48761, 48060))
    ...
    try:
        ...
        print("ALL SHARED FACILITY-REFRESH TESTS PASSED")
    finally:
        js.shutdown()
