...
shared_cmd. Base storage total is exactly what the GAP-4 checksum sums
...
`craft_equip` shared_cmd (like buy/sell/transfer/craft_assign): the client
...
worlds move together.
  ...
  HOST-AUTHORITY  the client loads rifles onto its craft through the REAL
                  ...
                  Only a shared_cmd can move the HOST's own base stores; a
...
Run:  python tools/coop_test/test_shared_equip.py
"""
...
def _chk(gc):
    return gc.ok({"cmd": "shared_checksum"})
...
def main():
    js = shared_fixture.bring_up("jequip", (48820, 48821, 48120))
    ...
    try:
        ...
        # not asserted, because in the FIXED path an in-flight shared_apply is a
        ...
        #      shared_cmd can do that; a replica-local mutation never reaches the
        ...
        #      shared_apply lands, an in-flight skew the 3s debounce absorbs (the
        ...
        js.finish()
        print("ALL SHARED EQUIP TESTS PASSED")
    finally:
        js.shutdown()
