"""PRD-J02: SHARED resume - the host re-streams the authoritative world.

1. Bring up a SHARED campaign (client adopts a streamed replica), remember the
   host world (base name/coords + funds), host saves, both quit.
2. Host reloads the SHARED save from the menu -> resume lobby.
...
Run:  python tools/coop_test/test_shared_resume.py
"""
...
def main():
    ...
    js = shared_fixture.bring_up("jres", (48670, 48671, 47970))
    host, client = js.host, js.client
    host_dir, client_dir = js.host_dir, js.client_dir
    try:
        # the pre-save world is already one shared world
        shared_fixture.assert_world_equal(host, client, "before save")
        ...
        # host saves the SHARED world to disk (host is the single authority)
        ...
        # host reloads the SHARED save (fresh process, same dir); client rejoins
        # with a clean dir. resume_campaign drives the standard resume handshake;
        # in SHARED the host serializes the CURRENT world fresh and streams it.
        ...
        # the reloaded host still reports SHARED
        hm = host.ok({"cmd": "save_markers"})
        assert hm.get("campaignType") == SHARED, f"reloaded host not SHARED: {hm}"
        ...
        # replica reports SHARED + the intact roster, no mirror bases
        cm = client.ok({"cmd": "save_markers"})
        assert cm.get("campaignType") == SHARED, f"client replica not SHARED: {cm}"
        ...
        print("PASS replica reports SHARED, intact roster, no mirror bases")
