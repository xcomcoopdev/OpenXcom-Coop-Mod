"""PRD-J03: the SHARED economy command protocol (shared_cmd / shared_apply).

A SHARED campaign is one host-authoritative world. Every economy mutation is a
command to the host; the host validates against the authoritative world, applies,
debits funds, and broadcasts shared_apply (carrying the new authoritative funds)
to all players. Replicas apply ONLY from shared_apply. This test exercises the
reference command "buy" through the real PurchaseState OK path:
  ...
  AC1c Insufficient funds -> shared_fail reason surfaces on the initiating
       client (popup + reason), world UNCHANGED on both sides.
  AC2  shared_cmd for an unknown cmd -> shared_fail, no crash (both instances
       stay alive and responsive).

Run:  python tools/coop_test/test_shared_purchase.py
"""
...
def main():
    ...
    try:
        ...
        # the mutation only lands after the shared_cmd -> shared_apply round-trip.
        host.wait_for("host applied client buy",
                      lambda: _rifles(host) == 5 or None, timeout=30, interval=0.5)
        client.wait_for("client applied shared_apply",
        ...
        client.wait_for("client received shared_fail",
        ...
        print(f"PASS AC1c insufficient funds: shared_fail '{cs['lastFail']}' surfaced, world unchanged")
        try:
            client.ok({"cmd": "coop_dialog_back"})  # dismiss the popup
        except Exception:
            pass

        # ---- AC2: unknown command -> shared_fail, no crash ------------------
        host.ok({"cmd": "shared_reset_stats"})
        client.ok({"cmd": "shared_reset_stats"})
        client.ok({"cmd": "shared_cmd", "jcmd": "bogus_frobnicate", "baseId": 0})
        client.wait_for("client received shared_fail for unknown cmd",
                        lambda: _stats(client)["failCount"] >= 1 or None, timeout=30, interval=0.5)
        cs = _stats(client)
        ...
        print(f"PASS AC2 unknown cmd: shared_fail '{cs['lastFail']}', both instances alive")
        try:
        ...
        print("ALL SHARED PURCHASE TESTS PASSED")
    finally:
        js.shutdown()
