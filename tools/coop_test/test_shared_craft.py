"""PRD-J08: SHARED shared craft command, interception + dogfights.

A SHARED campaign is one host-authoritative world: ANY player commands ANY
craft. Orders ride the J03 shared_cmd protocol (craft_launch / craft_retarget /
...
shared:true position snapshot. Dogfights (PRD-DF01, superseding the J08 initiator
...
on the HOST only; the shared snapshot materializes the UFO on the replica.

Run:  python tools/coop_test/test_shared_craft.py
"""
...
def main():
    js = shared_fixture.bring_up("jcraft", (48760, 48761, 48060))
    ...
    try:
        ...
        assert btn["buttons_visible"] is True, \
            f"client craft-command buttons hidden in SHARED: {btn}"
        ...
        print("ALL SHARED CRAFT TESTS PASSED")
    finally:
        js.shutdown()
