"""Playtest bugs (JOINT geoscape craft arrival):
  ...
  A. The "craft reached destination" alert (CraftPatrolState) is not shown when a craft
     ...
     directed the craft. In JOINT only the host runs the sim; the waypoint-arrival popup
...
Run:  python tools/coop_test/test_joint_waypoint_arrival.py
"""
...
def main():
    js = joint_fixture.bring_up("jwaypt", (48808, 48809, 48108))
    ...
    try:
        ...
        # arrives quickly). In JOINT this rides craft_retarget{targetType:point} ->
        ...
        js.finish()
        print("ALL JOINT WAYPOINT-ARRIVAL TESTS PASSED")
    finally:
        js.shutdown()
