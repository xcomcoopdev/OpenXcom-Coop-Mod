...
THE FIX routes each through a new host-authoritative shared_cmd carrying the
...
Each screen is its OWN bring-up so it can be red/green-captured in isolation:
    ...
    python tools/coop_test/test_shared_equip2.py            # all three
    python tools/coop_test/test_shared_equip2.py weapon     # just CraftWeaponsState
    python tools/coop_test/test_shared_equip2.py armor      # just SoldierArmorState
    python tools/coop_test/test_shared_equip2.py deequip    # just CraftArmorState
...
HOST-AUTHORITY  the client drives the REAL screen path (a `craft_rearm` /
                ...
                shared_cmd can move the HOST's own base stores, so the host's stock
...
def _chk(gc):
    return _wait_ok(gc, {"cmd": "shared_checksum"})["chkItems"]


def _resync(gc):
    return _wait_ok(gc, {"cmd": "shared_resync_stats"})
...
def test_weapon(ports):
    js = shared_fixture.bring_up("jrearm", ports)
...
def test_armor(ports):
    js = shared_fixture.bring_up("jsarmor", ports)
    host, client = js.host, js.client
    try:
        for gc in (host, client):
...
def test_deequip(ports):
    js = shared_fixture.bring_up("jcarmor", ports)
    host, client = js.host, js.client
    try:
...
def main():
    ...
    if which:
        ...
        for name in ("weapon", "armor", "deequip"):
            ...
            print(f"\n==== GAP-5b section: {name} ====")
            fn(ports)
    print("\nALL SHARED EQUIP2 (GAP-5b) TESTS PASSED")
