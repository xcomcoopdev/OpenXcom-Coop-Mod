"""PRD-J06: SHARED research start / allocate / cancel via shared_cmd.

A SHARED campaign is one host-authoritative world. The research screens mutate
NOTHING locally: starting a project, (re)allocating scientists, and cancelling a
project each ride the J03 shared_cmd protocol. The host validates against the live
world, applies, and broadcasts shared_apply; replicas apply ONLY from shared_apply.
Completion stays host-driven (J04 research_done).
  ...
  OVER     res_alloc more scientists than are free -> shared_fail, pools unchanged.
...
Run:  python tools/coop_test/test_shared_research.py
"""
...
def main():
    js = shared_fixture.bring_up("jres", (48720, 48721, 48020))
    ...
    try:
        ...
        r = client.ok({"cmd": "shared_cmd", "jcmd": "res_alloc", "baseId": 0,
        ...
        host.ok({"cmd": "shared_cmd", "jcmd": "res_alloc", "baseId": 0,
                 "payload": {"project": T, "assigned": A}})
        client.ok({"cmd": "shared_cmd", "jcmd": "res_alloc", "baseId": 0,
        ...
        for i, n in enumerate(burst):
            (host if i % 2 == 0 else client).ok(
                {"cmd": "shared_cmd", "jcmd": "res_alloc", "baseId": 0,
        ...
        host.ok({"cmd": "shared_reset_stats"})
        client.ok({"cmd": "shared_reset_stats"})
        client.ok({"cmd": "shared_cmd", "jcmd": "res_cancel", "baseId": 0,
                   "payload": {"project": T2}})
        ...
        #    free the assigned scientists (J04 path, on a SHARED-started project).
        # ================================================================
        ...
        print("ALL SHARED RESEARCH TESTS PASSED")
    finally:
        js.shutdown()
