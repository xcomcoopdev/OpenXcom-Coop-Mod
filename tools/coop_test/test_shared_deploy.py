"""PRD-J09 AC1 - SHARED mixed-owner squad assembly (pre-battle).

In a SHARED campaign there is ONE shared base / craft / roster. This test proves:

  1. Both players see the FULL roster at the shared base (the SEPARATE
     guest-filter is fenced off in SHARED).
...
Run:  python tools/coop_test/test_shared_deploy.py
...
def main():
    js = shared_fixture.bring_up("jdep", (48760, 48761, 48060))
    ...
    try:
        ...
        try:
            for tag, gc in (("host", host), ("client", client)):
                ...
                print(f"  DBG {tag} shared_stats: {gc.cmd({'cmd': 'shared_stats'})}")
