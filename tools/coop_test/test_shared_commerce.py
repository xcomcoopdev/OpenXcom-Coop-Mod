"""PRD-J05: SHARED economy UI - sell, hire (with ownership), transfer, containment.

A SHARED campaign is one host-authoritative world. Every inventory/funds-mutating
commerce screen routes through the J03 shared_cmd protocol: the client sends a
command, the host re-validates + re-prices against the live world, applies, and
broadcasts shared_apply (carrying the new authoritative funds). Replicas apply
ONLY from shared_apply. This test drives the REAL Basescape screens
(PurchaseState / SellState / ManageAlienContainmentState / TransferItemsState).
  ...
  SELL-FAIL host then tries to sell MORE than remain -> shared_fail (atomic, no
...
Run:  python tools/coop_test/test_shared_commerce.py
"""
...
def main():
    js = shared_fixture.bring_up("jcom", (48700, 48701, 48000))
    ...
    try:
        ...
        r = client.ok({"cmd": "shared_transfer", "item": PISTOL, "count": 6,
        ...
        print("ALL SHARED COMMERCE TESTS PASSED")
    finally:
        js.shutdown()
