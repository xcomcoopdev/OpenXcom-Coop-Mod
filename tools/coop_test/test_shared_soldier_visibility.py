...
A SHARED campaign holds ONE shared roster on both machines (the streamed world), so
...
soldier-list SCREEN (SoldiersState): in SHARED it filters nothing, so BOTH players
...
Run:  python tools/coop_test/test_shared_soldier_visibility.py
"""
...
def main():
    js = shared_fixture.bring_up("jsvis", (48860, 48861, 48160))
