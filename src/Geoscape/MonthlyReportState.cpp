...
void MonthlyReportState::btnOkClick(Action *)
{
	if (!_gameOver)
	{
		...
		// Autosave
		if (_game->getSavedGame()->isIronman() && (_game->getCoopMod()->getCoopStatic() == false || _game->getCoopMod()->getServerOwner() == true))
		{
			_game->pushState(new SaveGameState(OPT_GEOSCAPE, SAVE_IRONMAN, _palette));
		}
		else if (Options::autosave && (_game->getCoopMod()->getCoopStatic() == false || _game->getCoopMod()->getServerOwner() == true))
		{
